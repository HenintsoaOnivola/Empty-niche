...
the_folder='./Results_cross_guild/';
...
for log_sig_m=log_sig_m_range
    ...
    last_tr_an=read_last_line_to_array(strcat(the_folder,'animal_traits_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'));
    last_tr_pl=read_last_line_to_array(strcat(the_folder,'plant_traits_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'));
    last_pop_an=read_last_line_to_array(strcat(the_folder,'animal_abundance_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'));
    last_pop_pl=read_last_line_to_array(strcat(the_folder,'plant_abundance_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'));    
    
    intra_res(log_sig_m_range==log_sig_m)=k_x(kx,x0,sig_an,the_an_tr);
    mutu_res(log_sig_m_range==log_sig_m)=mutu(the_an_tr,last_tr_pl,last_pop_pl,const,sig_m,h);
    comp(log_sig_m_range==log_sig_m)=1-(competition_term_mutant(the_an_tr,last_tr_an,last_pop_an,sig_c_an)/intra_res(log_sig_m_range==log_sig_m));
    fit(log_sig_m_range==log_sig_m)=fitness_land_function(the_an_tr,last_pop_an,last_pop_pl,last_tr_an,last_tr_pl,rA,h,const,sig_c_an,sig_an,x0,kx,sig_m);
    sp_number(log_sig_m_range==log_sig_m)=length(last_tr_an);
    sp_abundance(log_sig_m_range==log_sig_m)=mean(last_pop_an);
    all_abundance=[all_abundance last_pop_an last_pop_pl];
end
figure('Visible','off');
plot(log_sig_m_range,comp,'-','Color',rgb('magenta'),'LineWidth',1.5);
set(gca,'YColor',rgb('magenta'));
hold on;
the_max=max([fit mutu_res comp]);
the_min=min([fit mutu_res comp]);
set(gca,'Ylim',[the_min the_max]);
plot([-1.9375 log(the_sigma) -1.5625],[the_min the_min the_min],'kx','MarkerSize',12,'LineWidth',1.5);
hold on;
yline(0,'--');
addaxis(log_sig_m_range,mutu_res,[the_min the_max],'-','Color',rgb('MediumSeaGreen'),'LineWidth',1.5);
addaxis(log_sig_m_range,fit,[the_min the_max],'-','Color',[0 0 0 0.75],'LineWidth',1.5);
addaxis(log_sig_m_range,sp_number,'o','Color',rgb('orange'),'MarkerFaceColor',rgb('orange'));
addaxis(log_sig_m_range,sp_abundance,'o','Color',[0.6 0.6 0.6],'MarkerFaceColor',[0.6 0.6 0.6]);
...
plot_name=strcat(the_folder_output,'/FigB4j.png');
...
myfiguresize = [left, bottom, width*3, height];
set(gcf,'PaperPosition', myfiguresize);
print(plot_name,'-dpng','-r300');
clf;

%%Plot fitness, and traits evo and fitness components for particular cases
log_sig_m_range=[-1.9375 the_sigma -1.5625];
sc=@(x) sign(x).*(log10(1+1000*abs(x)));
min_ab=min(all_abundance);
max_ab=max(all_abundance);
for log_sig_m=log_sig_m_range
    if log_sig_m==the_sigma
        sig_m=log_sig_m;
        res=more_simulation_for_supp_mat(sig_c_an,sig_m,'mutu');
        time_plot=res{1};
        trait_plot_an=res{2};
        trait_plot_pl=res{3};
        pop_plot_an=res{4};
    ...
    for pl_ind=1:length(tr_pl_last)
        if min_ab==max_ab
            ...
            ms=4+((ab_pl_last(an_ind)-min_ab)*(10-4)/(max_ab-min_ab));
        end
    ...
    plot_name=strcat(the_folder_output,'/FigB4_fitness_sigM=',num2str(sig_m),'.png');
    ...
    plot_name=strcat(the_folder_output,'/FigB4_fitness_component_sigM=',num2str(sig_m),'.png');
    %Here we preserve the size of the image when we save it.
    set(gcf,'InvertHardcopy','on');
    set(gcf,'PaperUnits', 'inches');
    papersize = get(gcf, 'PaperSize');
    ...
    the_fig=plot_traits_with_fitness_bg_plant(trait_interval,sepIndex,time_plot,trait_plot_an,trait_plot_pl,pop_plot_an,pop_plot_pl,rA,const,h,x0,kx,sig_an,sig_c_an,sig_m,the_an_tr);
    plot_name=strcat(the_folder_output,'/FigB4_traits_evo_sigM=',num2str(sig_m),'.png');
    print(plot_name,'-dpng','-r300');
    clf;
