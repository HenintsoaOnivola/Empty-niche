function res=invasion_different_scenario(rA,rP,h,const,log_sig_c_an,x0,y0,kx,ky,log_sig_m,opt_resource)
    mutant_fraction=0.1;
    invader_fraction=0.01;

    if strcmp(opt_resource,'intra')
        the_fold=strcat('./Results_intra_guild/');
        other_dest=strcat('./Results_invasion_intra_guild/');
    end
    if strcmp(opt_resource,'cross')
        the_fold=strcat('./Results_cross_guild/');
        other_dest=strcat('./Results_invasion_cross_guild/');
    end
    ...
    if EN_value==0  %%when there is no empty niches
       ...
       disp ('no empty niche');
        ...
        if ~isempty(the_interval_an)
            %see in which trait interval was the introduction in the other
            %scenario and make an introduction at another trait
            %interval
            other_intros=read_file_to_array(strcat(other_dest,'introIndex_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'));
            other_intros=other_intros(1);
            other_an_tr=read_data_into_cell(strcat(other_dest,'animal_traits_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'));
            other_introduced=other_an_tr{other_intros};
            other_introduced=other_introduced(end);
            inter_ind=find((the_interval_an(2:end)-the_interval_an(1:end-1))>0.01);
            inter_ind=inter_ind(1);
            if other_introduced<=the_interval_an(inter_ind)
                the_new_interval_an=the_interval_an(inter_ind+1:end);
            else
                the_new_interval_an=the_interval_an(1:inter_ind);
            end
            the_interval_an=the_new_interval_an;
        ...
        if ~isempty(the_interval_pl)
            ...
            other_intros=read_file_to_array(strcat(other_dest,'introIndex_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'));
            other_intros=other_intros(1);
            other_pl_tr=read_data_into_cell(strcat(other_dest,'plant_traits_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'));
            other_introduced=other_pl_tr{other_intros};
            other_introduced=other_introduced(end);
            inter_ind=find((the_interval_pl(2:end)-the_interval_pl(1:end-1))>0.01);
            inter_ind=inter_ind(1);
            if other_introduced<=the_interval_pl(inter_ind)
                the_new_interval_pl=the_interval_pl(inter_ind+1:end);
            else
                the_new_interval_pl=the_interval_pl(1:inter_ind);
            end
            the_interval_pl=the_new_interval_pl;
