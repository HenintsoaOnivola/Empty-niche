...
for log_sig_c_an=log_sig_c_an_range
        ...
        while ( (convergence==false) && count_integrate<=infinite )
            ...
            if isempty (IE)
                ...
                if any(IE==1)
                    ...
                    if any (IE==2) 
                        ...
                        while(temp_total_time<=10^(6))
                            ...
                            if (~isempty(temp_IE) && any (temp_IE==3))  || count_temporary>20
                                ind_oscillation=detect_oscillation_using_trait_variation_RC(temp_sol,animal);
                    ...
                    if any (IE==3)
                        %detect oscillation
                        ind_oscillation=detect_oscillation_using_trait_variation_RC(sol,animal);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    mkdir('Results_RC');
    clear_files(strcat('./Results_RC/time_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'.dat'));
    clear_files(strcat('./Results_RC/animal_traits_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'.dat'));
    clear_files(strcat('./Results_RC/animal_abundance_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'.dat'));
    clear_files(strcat('./Results_RC/sepIndex_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'.dat'));
