function plot_fitness_background_function_RC(sep_index,time_plot,an_traits,ab_an,Pareto_an,r,sig_c_an,sig_a,x0,kx,trait_limits,x_tick_lab)
    ...
    for tt=2:length(time_plot)
        if length(an_traits{tt})~=an_before 
    ...
    for jj=1:1:length(time_plot)
        ff_an=fitness_land_function_RC(trait_array,transpose(ab_an{jj}),transpose(an_traits{jj}),r,sig_c_an,sig_a,x0,kx);
    ...
    all_max=(max(the_matrix_land_an));
    considered_max=max(all_max);
    considered_max=min([considered_max 1.1]);
    all_min=min(min(the_matrix_land_an)) ;
    ...
    the_min_ab=min(cellfun(@(x) min(x(:)),ab_an)) ;
    the_max_ab=max(cellfun(@(x) max(x(:)),ab_an)) ;
