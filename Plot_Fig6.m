...
%along time in two folders: 'Selection_pressure_intra' and
%'Selection_pressure_cross'. Saving the results in these folders is to
...
for each_opt=1:length(res_opt)
    ...
    for log_sig_c_an=log_sig_c_an_range
        for log_sig_m=log_sig_m_range
            ...
            re=detect_EN_function(strcat('./Results_',res_opt{each_opt},'_guild/'),log_sig_an,log_sig_c_an,log_sig_m,rA,rP,const,h,x0,y0,kx,ky);

            if re{1}==1
   
                the_folder=strcat('./Results_invasion_',res_opt{each_opt},'_guild/');
                the_dest=strcat('./Selection_pressure_',res_opt{each_opt},'_guild/');
...
%This second part of the code plots Fig6 by reading data from those files
%generated from part 1. If all the necessary data have already been
%generated, one can only run this second part of the code
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

sel_folder_res='./Selection_pressure_intra_guild/';
sel_folder_mutu='./Selection_pressure_cross_guild/';
tr_folder_res='./Results_invasion_intra_guild/';
tr_folder_mutu='./Results_invasion_cross_guild/';
...
for log_sig_c_an=log_sig_c_an_range
    for log_sig_m=log_sig_m_range
        ...
        if log_sig_m<-1.5 %case of two evolutionary stability
            if isfile(file_res)  %take only those that have introduced species after empty niches, intra-guild case
        ...
        else  %case where there is only one evolutionary stable point
          if isfile(file_res)
            an_res=dlmread_empty(file_res,'\t',0,0,NaN);
            an_res=an_res(:,1:end-1);
            pl_res=dlmread_empty(strcat(sel_folder_res,'pl_sel_res_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m),'.dat'),'\t',0,0,NaN);
...
%Plotting Fig6a (early introduction for resident species)
figure('Visible','off');
colormap(my_colormap);
pp_res=pcolor((res_early_matrix));
hold on;
...
dis_y=nansum(res_early_matrix,1);
dis_y=dis_y(1:end-1);
new_dis_y=(dis_y/max(dis_y))*(500/6);
plot(1:1:500,new_dis_y,'-','Color',rgb('Black'),'LineWidth',1);
...
box off;
name=strcat('./Plots/Fig6a.png');
print(name,'-dpng','-r600');

%Plotting Fig6c (intermediate introduction for resident species)
figure('Visible','off');
colormap(my_colormap);
pp_res=pcolor(res_int_matrix);
hold on;
set(pp_res,'edgecolor','none');
set(gca,'TickDir','in','TickLength',[0.015 0.015],'XMinorTick','off','YMinorTick','off','layer','top','LineWidth',1.5);
set(gca,'Xtick',the_x_ticks,'XtickLabel',the_x_ticks_label,'Ytick',the_y_ticks,'YTickLabel',the_y_ticks_label,'FontSize',15);
...
set(ax2,'TickDir','in','TickLength',[0.015 0.015],'XMinorTick','off','YMinorTick','off','layer','top','LineWidth',1.5,'Xdir','reverse','FontSize',15);
box off;
name=strcat('./Plots/Fig6c.png');
print(name,'-dpng','-r600');

%Plotting Fig6e (late introduction for resident species)
figure('Visible','off');
colormap(my_colormap);
pp_res=pcolor(res_late_matrix);
hold on;
set(pp_res,'edgecolor','none');
set(gca,'TickDir','in','TickLength',[0.015 0.015],'XMinorTick','off','YMinorTick','off','layer','top','LineWidth',1.5);
set(gca,'Xtick',the_x_ticks,'XtickLabel',the_x_ticks_label,'Ytick',the_y_ticks,'YTickLabel',the_y_ticks_label,'FontSize',15);
...
new_dis=new_dis/max(new_dis);
plot(new_dis,1:5:500,'-','Color',rgb('ForestGreen'),'LineWidth',0.8,'Parent',ax2);
set(ax2,'Ytick',the_y_ticks,'YTickLabel',[],'Color','none','XAxisLocation','top','YAxisLocation','right','XColor',rgb('ForestGreen'),'YColor','k');
set(ax2,'TickDir','in','TickLength',[0.015 0.015],'XMinorTick','off','YMinorTick','off','layer','top','LineWidth',1.5,'Xdir','reverse','FontSize',15);
box off;

name=strcat('./Plots/Fig6e.png');
print(name,'-dpng','-r600');
 
%Plotting Fig6b (early introduction for introduced species)
figure('Visible','off');
colormap(my_colormap);
...
name=strcat('./Plots/Fig6b.png');
print(name,'-dpng','-r600');

%Plotting Fig6d (intermediate introduction for introduced species)
figure('Visible','off');
colormap(my_colormap);
...
name=strcat('./Plots/Fig6d.png');
print(name,'-dpng','-r600');

%Plotting Fig6f (late introduction for introduced species)
figure('Visible','off');
colormap(my_colormap);
