function res=simulation_invasion_different_threshold_sel_grad(sig_c_an,sig_m,lambda,pre_an_tr,pre_pl_tr,pre_an_ab,pre_pl_ab,pre_time,pre_separating_index)
    ...
    while invade==true && count_introduction<max_introduction && repetition==false %&& oscillation==false
        ...
        while (new_sys==true && count_into_while<maximum_branching)
            ...
            while ( (convergence==false) && count_integrate<=infinite )
                tic;
                [time,sol,TE,YE,IE]=solve_eco_evo_subsequent_branching_lambda(ini_val,rA,rP,h,const,sig_c_an,sig_c_pl,sig_an,sig_pl,x0,y0,kx,ky,sig_m,animal,plant,temporary_time,count_integrate,count_into_while,lambda);
                        ...
                        if any (IE==2) 
                            ...
                            if count_into_while<maximum_branching+1
                                ...
                                while(temp_total_time<=10^(6))
                                    ...
                                    tic;
                                    [temp_time,temp_sol,temp_TE, temp_YE, temp_IE]=solve_eco_evo_subsequent_branching_lambda(initial_condition,rA,rP,h,const,sig_c_an,sig_c_pl,sig_an,sig_pl,x0,y0,kx,ky,sig_m,animal,plant,temporary_time,count_temporary,count_into_while,lambda);
        ...
        the_inter=localize_positive_fitness(last_pop_an,last_pop_pl,last_tr_an,last_tr_pl,rA,rP,h,const,log(sig_c_an),log(sig_c_pl),sig_an,sig_pl,x0,y0,kx,ky,log(sig_m));
