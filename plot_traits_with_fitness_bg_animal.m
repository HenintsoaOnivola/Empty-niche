function figg=plot_traits_with_fitness_bg_animal(trait_inter,sep_index,time_plot,an_traits,pl_traits,ab_an,ab_pl,rA,const,h,x0,kx,sig_an,sig_c_an,sig_m,the_an_tr)
    ...
    the_matrix_land_an=zeros(length(consid_time)+1,length(trait_inter)+1);
    each_col=1;
    for jj=1:1:length(time_plot)
        ff_an=fitness_land_function(trait_inter,transpose(ab_an{jj}),transpose(ab_pl{jj}),transpose(an_traits{jj}),transpose(pl_traits{jj}),rA,h,const,sig_c_an,sig_an,x0,kx,sig_m);
    ...
    considered_max=max(max(the_matrix_land_an));
    considered_max=min([considered_max 1.1]);
    considered_min=min(min(the_matrix_land_an));
    ...
    the_min=min(min(the_matrix_land_an));
    the_max=max(max(the_matrix_land_an));
    
    the_matrix_land_an=double((the_matrix_land_an-the_min)/(the_max-the_min));
 
    pp1=pcolor([trait_inter trait_inter(end)],transpose([consid_time; consid_time(end)+(consid_time(end)-consid_time(end-1))]),the_matrix_land_an);
    hold on;
    %caxis(gca,[0 2.1]);
...
%     the_matrix_land_pl(:,end)=NaN;
%     
%     pp2=pcolor([trait_inter trait_inter(end)],transpose([consid_time; consid_time(end)+(consid_time(end)-consid_time(end-1))]),the_matrix_land_pl);
%     hold on;
%     caxis(gca,[0 2.1]);
%     set(pp2,'edgecolor','none');
    ...
    plot(the_an_tr,0,'kx','Markersize',12);
