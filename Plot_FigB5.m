...
%Plot the last figure (fitness component over tolerance to trait
%difference)
log_sig_c_an=-2.3125;
small_range=-2.5:((0.0625)/10):-2.4375;
small_range=small_range(2:end-1);
log_sig_m_range=[-3:0.0625:-2.5 small_range -2.4375:0.0625:-0.5] ;
the_pl_tr=1.85;
...
for log_sig_m=log_sig_m_range
    if ismember(log_sig_m,small_range) 
        sig_m=exp(log_sig_m);
        ...
        sig_m=exp(log_sig_m);
        last_tr_an=read_last_line_to_array(strcat(the_folder,'animal_traits_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m,6),'.dat'));
        last_tr_pl=read_last_line_to_array(strcat(the_folder,'plant_traits_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m,6),'.dat'));
        last_pop_an=read_last_line_to_array(strcat(the_folder,'animal_abundance_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m,6),'.dat'));
        last_pop_pl=read_last_line_to_array(strcat(the_folder,'plant_abundance_logsigA_',num2str(log_sig_an),'_logsigC',num2str(log_sig_c_an),'_logsigM',num2str(log_sig_m,6),'.dat'));    
    end
...
figure('visible','off');
plot(log_sig_m_range(comp>0),comp(comp>0),'-','Color',rgb('magenta'),'LineWidth',1.5);
...
plot(log_sig_m_range(comp<=0),comp(comp<=0),'-','Color',rgb('magenta'),'LineWidth',1.5);
plot([-2.5625 -2.25],[the_min the_min],'kx','MarkerSize',12,'LineWidth',1.5);
yline(0,'--');
addaxis(log_sig_m_range,mutu_res,[the_min the_max],'-','Color',rgb('MediumSeaGreen'),'LineWidth',1.5);
addaxis(log_sig_m_range(fit>0.1),fit(fit>0.1),[the_min the_max],'-','Color',[0 0 0 0.75],'LineWidth',1.5);
addaxisplot(log_sig_m_range(fit<=0.1),fit(fit<=0.1),3,'-','Color',[0 0 0 0.75],'LineWidth',1.5)
...
plot_name=strcat(the_folder_output,'/FigB5g.png');
...
for log_sig_m=log_sig_m_range
    ...
    for pl_ind=1:length(tr_pl_last)
        ...
        all_ms=[all_ms ms];
        plot(tr_pl_last(pl_ind),sc(fitness_land_function(tr_pl_last,ab_pl_last,ab_an_last,tr_pl_last,tr_an_last,rP,h,const,sig_c_pl,sig_pl,y0,ky,sig_m)),'ro','MarkerSize',ms,'MarkerFaceColor','r');
    end
    ...
    if log_sig_m==min(log_sig_m_range)
        [~, hobj, ~, ~] =legend([p1,p2],'Animals','Plants','FontSize',12,'Location','SouthWest');
        hl = findobj(hobj,'type','line');
        set(hl,'LineWidth',1.2);
    end
    plot_name=strcat(the_folder_output,'/FigB5_fitness_sigM=',num2str(sig_m),'.png');
    ...
    plot_name=strcat(the_folder_output,'/FigB5_fitness_component_sigM=',num2str(sig_m),'.png');
    %Here we preserve the size of the image when we save it.
    set(gcf,'InvertHardcopy','on');
    set(gcf,'PaperUnits', 'inches');
    papersize = get(gcf, 'PaperSize');
    ...
    plot_name=strcat(the_folder_output,'/FigB5_traits_evo_sigM=',num2str(sig_m),'.png');
    print(plot_name,'-dpng','-r300');
    clf;
