function [time, solution,TE,YE,IE]=solve_eco_evo_subsequent_branching_lambda(INI,rA,rP,h,const,sig_c_an,sig_c_pl,sig_an,sig_pl,x0,y0,kx,ky,sig_m,animal,plant,past_time,the_count_integrate,count_sys,lambda)
    ...
    function [value,isterminal,direction]=events(t,y)
        ...
        density=y(1:animal+plant);
        traits=y(animal+plant+1:length(y));
        traits_an=traits(1:animal);
        traits_pl=traits(animal+1:end);
        pop_an=density(1:animal);
        pop_pl=density(animal+1:end);
        ...
        the_threshold=lambda;
        
        for i=1:(animal+plant)
        ...
        if count_sys==1
            ...
            if count_conv==(animal+plant)  && past_time+t>100
            ...
            if count_conv==(animal+plant)  %&& past_time+t>4*10^4
                value(2)=0;
            else
                value(2)=1;
            end
    ...
    function dxdt=ode_function_model(t,IN)
        ...
        A=IN(1:animal);
        P=IN(animal+1:animal+plant);
        x=IN(animal+plant+1:2*animal+plant);
        y=IN(2*animal+plant+1:2*animal+2*plant);
